...
```
pack build python-function-microservice1 --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.0.10
```

Where `python-function-microservice1` is the name of your runnable function image, later used by Docker.
...
```
docker run -it --rm -p 8080:8080 python-function-microservice1
```
...
```
tanzu apps workload create python-function-microservice1 -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/python-function-microservice1-source \
  --type web
```
...
```
tanzu apps workload get python-function-microservice1
```
